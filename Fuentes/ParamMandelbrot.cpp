...
__fastcall TVCap_Mandelbrot::TVCap_Mandelbrot(TComponent* Owner) : TForm(Owner)
{
   Aceptar = false;
}

// Al presionar el boton de acptar
void __fastcall TVCap_Mandelbrot::Button1Click(TObject *Sender)
{
   Aceptar = true;
   Close();        
}
...
void __fastcall TVCap_Mandelbrot::ListBoxParametrosClick(TObject *Sender)
{
...
void __fastcall TVCap_Mandelbrot::EditValorParametroKeyPress(TObject *Sender, char &Key)
{
   if (Key == VK_RETURN) {
      Key = 0;
